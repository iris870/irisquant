...
class MeanReversionBacktest:
    ...
    def backtest(self, klines_4h):
        ...
        for i in range(pwl_lookback + 50, len(klines_4h) - 2):
            # 取前一周K线（排除当前周）
            ...
            # 下一根K线
            next1 = klines_4h[i+1]
            next1_close = float(next1[4])
            next2 = klines_4h[i+2]
            next2_close = float(next2[4])
            ...
            # 信号检测：允许后面2根K线内回归
            buy_signal = (current_close > pwl or next1_close > pwl or next2_close > pwl)
            sell_signal = (current_close < pwh or next1_close < pwh or next2_close < pwh)
